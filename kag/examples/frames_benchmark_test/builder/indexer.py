...
class FramesCorpusReader(SourceReaderABC):
    @property
...
class FramesBuilderChain(BuilderChainABC):
    def build(self, **kwargs):
        project_id = int(os.getenv("KAG_PROJECT_ID"))
        source = FramesCorpusReader()
...
def buildKB(corpusFilePath):
    FramesBuilderChain().invoke(file_path=corpusFilePath, max_workers=20)

    logger.info(f"\n\nbuildKB successfully for {corpusFilePath}\n\n")


if __name__ == '__main__':
    filePath = "./data/frames_corpus.json"
